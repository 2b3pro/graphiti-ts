...
import type { EdgeDuplicate, ExtractedEdge, ExtractedEdges } from '../prompts/models';
...
import { capStringAttributes } from './attribute-guards';
...
export async function extractEdges(
	...
	const llmResponse = await callGenerateResponse(
		llmClient,
	...
	const allEdgesData: ExtractedEdge[] = (llmResponse as unknown as ExtractedEdges).edges ?? [];
...
export async function resolveExtractedEdge(
	...
	try {
		...
		if (relatedEdges.length === 0 && existingEdges.length === 0) {
			if (edgeTypeCandidates?.[extractedEdge.name]?.fields) {
				...
				if (Object.keys(fields).length > 0) {
					const attrs = await extractEdgeAttributes(
						llmClient,
						extractedEdge,
						episode,
						fields,
		...
		for (let i = 0; i < existingEdges.length; i++) {
			...
			if (extractedEdge.source_node_uuid === existing.source_node_uuid)
				sharedEntities.push('source');
			if (extractedEdge.target_node_uuid === existing.target_node_uuid)
				sharedEntities.push('target');
			if (extractedEdge.source_node_uuid === existing.target_node_uuid)
				sharedEntities.push('source-target');
			if (extractedEdge.target_node_uuid === existing.source_node_uuid)
				sharedEntities.push('target-source');
...
export function resolveEdgeContradictions(
	...
	for (const edge of invalidationCandidates) {
		...
		if (edge.invalid_at && resolvedEdge.valid_at && edge.invalid_at <= resolvedEdge.valid_at) {
			continue;
		}
		if (edge.valid_at && resolvedEdge.invalid_at && resolvedEdge.invalid_at <= edge.valid_at) {
			continue;
		}
...
async function extractEdgeAttributes(
	...
	const response = await callGenerateResponse(
		llmClient,
	...
	return capStringAttributes(response, schema);
}
...
async function getEdgesBetweenNodes(
	...
	return result.records.map((r) => ({
		...
		confidence:
			Array.isArray(r.confidence) && (r.confidence as number[]).length === 3
