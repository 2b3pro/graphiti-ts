...
import type { EmbedderClient, GraphDriver, GraphitiClients, LLMClient } from '../contracts';
import { type EntityNode, EpisodeTypes, type EpisodicNode } from '../domain/nodes';
...
function makeMockClients(llmResponses: Record<string, unknown>[]): GraphitiClients {
	return {
...
describe('extractNodes', () => {
	...
	test('excludes entities of excluded types', async () => {
		...
		const nodes = await extractNodes(clients, makeEpisode(), [], entityTypes, ['Bot']);
		expect(nodes).toHaveLength(1);
		expect(nodes[0]!.name).toBe('Alice');
	});
...
describe('resolveExtractedNodes', () => {
	test('pre-filter helper skips only low-score weak-margin candidates', () => {
		expect(
			shouldSkipNodeResolution([0.62, 0.6], {
		...
		expect(
			shouldSkipNodeResolution([0.62, 0.4], {
				pre_filter_enabled: true,
				node_similarity_threshold: 0.7,
				margin_threshold: 0.05,
			}),
	...
	test('resolves nodes against existing nodes via exact match', async () => {
		...
		const clients = makeMockClients([{ entity_resolutions: [] }]);
	...
	test('uses LLM for unresolved nodes', async () => {
		...
		const clients = makeMockClients([
			{
				entity_resolutions: [{ id: 0, name: 'Bob', duplicate_name: 'Alice Johnson' }],
			},
		]);
...
describe('extractAttributesFromNodes', () => {
	test('generates embeddings for nodes without embeddings', async () => {
		const clients = makeMockClients([{ summaries: [{ name: 'Alice', summary: 'A person' }] }]);
	...
	test('drops overlong and null-stand-in string attributes', async () => {
		const clients = makeMockClients([
			{
				occupation: 'engineer',
				industry: 'x'.repeat(251),
				phone: 'N/A',
				aliases: ['AJ', 'y'.repeat(251)],
			},
			{ summaries: [{ name: 'Alice', summary: 'A developer' }] },
		]);
	...
	test('honors schema maxLength for string attributes', async () => {
		const clients = makeMockClients([
			{ code: 'abcdef' },
			{ summaries: [{ name: 'Alice', summary: 'A developer' }] },
		]);
	...
	test('applies summaries from LLM to matching nodes', async () => {
		...
		const result = await extractAttributesFromNodes(clients, [node], makeEpisode());
