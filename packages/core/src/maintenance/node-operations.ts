...
import type { GenerateResponseOptions, GraphitiClients, LLMClient } from '../contracts';
import {
...
import type { EntityNode, EpisodeType, EpisodicNode } from '../domain/nodes';
...
import { MAX_SUMMARY_CHARS, truncateAtSentence } from '../utils/text';
...
export async function extractNodes(
	...
	let prompt: import('../prompts/types').Message[];
...
export async function extractAttributesFromNodes(
	...
	for (let i = 0; i < nodes.length; i++) {
		...
		if (attrs && Object.keys(attrs).length > 0) {
			const entityType =
				entityTypes?.[node.labels.find((label) => label !== 'Entity') ?? ''] ?? null;
			const cappedAttrs = capStringAttributes(attrs, entityType?.fields ?? null);
			if (Object.keys(cappedAttrs).length > 0) {
				node.attributes = {
					...(node.attributes ?? {}),
					...cappedAttrs,
				} as import('../domain/common').EntityAttributes;
			}
		}
	}
...
async function extractEntitySummariesBatch(
	...
	await semaphoreGather(
		flights.map(
			(flight) => () =>
				processSummaryFlight(llmClient, flight, episode, previousEpisodes, llmContext),
		),
	);
}
