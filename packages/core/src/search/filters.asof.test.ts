...
import { CypherIdentifierValidationError } from '@graphiti/shared';
...
describe('searchAsOf temporal filter construction', () => {
	...
	const filters = createSearchFilters({
		...
		invalid_at: [[{ date: asOf, comparison_operator: '>' }], [{ comparison_operator: 'IS NULL' }]],
	});
	...
	test('property filter names must be safe Cypher identifiers', () => {
		expect(() =>
			createSearchFilters({
				property_filters: [
					{
						property_name: 'name) DETACH DELETE e //',
						property_value: 'x',
						comparison_operator: '=',
					},
				],
			}),
		).toThrow(CypherIdentifierValidationError);
		...
		expect(() => edgeSearchFilterQueryConstructor(unsafeFilters, 'neo4j' as never)).toThrow(
			CypherIdentifierValidationError,
		);
	});
});
