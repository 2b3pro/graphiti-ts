import type { EntityEdge, EntityNode, EpisodicNode, Graphiti } from '@graphiti/core';
import { createSearchFilters, NODE_HYBRID_SEARCH_RRF } from '@graphiti/core';
...
export function createGraphitiMcpServer(graphiti: Graphiti, config: McpServerConfig): McpServer {
	...
	server.tool(
		...
		async ({ query, group_ids, max_nodes, entity_types }) => {
			const effectiveGroupIds = coerceGroupIds(group_ids, config.default_group_id);
	...
	server.tool(
		...
		async ({ uuid }) => {
			try {
				...
				return {
					content: [{ type: 'text' as const, text: `Error getting entity edge: ${String(error)}` }],
					isError: true,
				};
			}
		},
	...
	server.tool(
		...
		'Clear all data from the graph for specified group IDs.',
		{
			group_ids: z
				...
				.optional()
				.describe(
					'Optional group ID or list of group IDs to clear. Defaults to the configured default group.',
				),
		},
		async ({ group_ids }) => {
			const effectiveGroupIds = coerceGroupIds(group_ids, config.default_group_id);
