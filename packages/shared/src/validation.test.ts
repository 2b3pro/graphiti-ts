...
describe('validation', () => {
	...
	test('validates safe Cypher identifiers', () => {
		expect(validateCypherIdentifier('valid_name_1')).toBeTrue();
		expect(() => validateCypherIdentifier('bad-name')).toThrow(CypherIdentifierValidationError);
		expect(() => validateCypherIdentifier('1bad')).toThrow(CypherIdentifierValidationError);
	});
});
