...
import { GROUP_ID_PATTERN, SAFE_CYPHER_IDENTIFIER_PATTERN } from './graph';
...
export function validateCypherIdentifier(identifier: string): true {
	if (!SAFE_CYPHER_IDENTIFIER_PATTERN.test(identifier)) {
		throw new CypherIdentifierValidationError(identifier);
	}
